...
1. Simulate $u_i \sim U(0,1)$ $n$ times.
2. Compare each uniform random variable draw against $\theta$.

  - If $u_i\leq\theta$ then $y_i=1$
  - Else $y_i=0$
...
4. Accept all $y^*$ as from $f(x)$. Discard all $u_i$ and return to step 1.

``` {r }
u<-runif(10); u
theta<-0.3
y<-ifelse(u<=theta, 1, 0); y
y.star<-sum(y); y.star
```
